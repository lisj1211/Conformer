...
def predict_long_audio():
    start = time.time()
    result = predictor.predict_long(audio_data=args.wav_path, is_itn=args.is_itn)
    score, text = result['score'], result['text']
...
def real_time_predict_demo():
    ...
    while data != b'':
        ...
        result = predictor.predict_stream(audio_data=data, is_itn=args.is_itn, is_end=d == b'')
        data = d
