...
from src.data_utils.utils import read_manifest
...
class FeatureNormalizer(object):
    ...
    def compute_mean_istd(self,
        ...
        logger.info('开始抽取{}条数据计算均值和标准值...'.format(len(manifest)))
        dataset = NormalizerDataset(manifest, preprocess_configs)
        test_loader = DataLoader(dataset=dataset, batch_size=batch_size, collate_fn=collate_fn, num_workers=num_workers)
        with torch.no_grad():
            # 求总和
            std, means = None, None
            number = 0
            for std1, means1, number1 in tqdm(test_loader, desc='compute_mean_istd'):
                number += number1
                if means is None:
                    means = means1
                else:
