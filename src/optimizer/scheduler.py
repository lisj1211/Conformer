...
class CosineWithWarmup(_LRScheduler):
    def __init__(self, optimizer: torch.optim.Optimizer, T_max, eta_min=0, warmup_steps=None, warmup_ratio=0.1,
                 last_epoch=-1):
        """
...
if __name__ == '__main__':
    ...
    model = torch.nn.Conv2d(1, 3, 3, 3)

    optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)
    l1 = []
    s1 = WarmupLR(optimizer, warmup_steps=2000)
    for epoch in range(1, 20000):
        optimizer.zero_grad()
        optimizer.step()
        l1.append(optimizer.param_groups[0]['lr'])
        s1.step()

    optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)
    l2 = []
    s2 = NoamHoldAnnealing(optimizer, warmup_steps=2000)
    for epoch in range(1, 20000):
        optimizer.zero_grad()
        optimizer.step()
        l2.append(optimizer.param_groups[0]['lr'])
        s2.step()

    optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)
    l3 = []
    s3 = CosineWithWarmup(optimizer, T_max=20000)
    for epoch in range(1, 20000):
        optimizer.zero_grad()
        optimizer.step()
        l3.append(optimizer.param_groups[0]['lr'])
        s3.step()
